# From : https://github.com/mfaruqui/dwl/blob/master/src/rprop.py
...
def rprop_plus_updates(params, grads):
    ...
    # RPROP+ parameters
    updates = []
    deltas = 0.1*numpy.ones(len(params),dtype=numpy.float32)
    last_weight_changes = numpy.zeros(len(params),dtype=numpy.float32)
    last_params = params
    ...
    for param, gparam, last_gparam, delta, last_weight_change in \
        ...
        if T.gt(change, 0) :
            ...
            last_gparam = param
        # update the weights
        updates.append((param, T.cast(param - weight_change, dtype='float32')))
